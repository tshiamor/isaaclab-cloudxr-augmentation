...
"""
Single robot configuration for VR hand tracking data collection.
This config is optimized for Cloud XR with a single environment.
"""
...
@configclass
class FrankaJengaBowlCloudXRSingleEnvCfg(StackEnvCfg):
    """Single robot configuration for VR hand tracking."""

    def __post_init__(self):
        # post init of parent
        super().__post_init__()

        # **IMPORTANT: Set to 1 environment for VR hand tracking**
        ...
        # Adjust XR anchor position/rotation for  setup
        self.xr.anchor_pos = (-0.1, -0.5, -1.05)
        self.xr.anchor_rot = (0.866, 0, 0, -0.5)
